...
class SparseFlowAugmentorm:
    ...
    def resize_sparse_flow_exp_map(self, flow, dc_b, exp, valid, fx=1.0, fy=1.0):
        ht, wd = flow.shape[:2]
        coords = np.meshgrid(np.arange(wd), np.arange(ht))
        coords = np.stack(coords, axis=-1)
        ...
        flow = flow.reshape(-1, 2).astype(np.float32)
        exp = exp.reshape(-1).astype(np.float32)
        dc_b = dc_b.reshape(-1).astype(np.float32)
        ...
        dc_bimg = np.ones([ht1, wd1], dtype=np.float32)

        exp_img[yy, xx] = exp1
        flow_img[yy, xx] = flow1
        valid_img[yy, xx] = 1
        dc_bimg[yy, xx] = dc_b1
        return flow_img, valid_img, exp_img,dc_bimg

    def spatial_transform(self, img1, img2, flow, valid, dc,instance,dc_b):
        # randomly sample scale

        ht, wd = img1.shape[:2]
        min_scale = np.maximum(
            (self.crop_size[0] + 1) / float(ht),  #crop_size是什么含义
            (self.crop_size[1] + 1) / float(wd))
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            img1 = cv2.resize(img1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            instance = cv2.resize(instance, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST)
            # dc   = cv2.resize(dc  , None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST)
            flow, valid, exp,dc_b = self.resize_sparse_flow_exp_map(flow,dc_b, dc[:, :, 0], valid, fx=scale_x, fy=scale_y)
            dc_out = np.ones_like(flow)
            dc_out[:, :, 0] = exp
            dc_out[:, :, 1] = valid
        else:
        ...
        dc_out = dc_out[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        valid = valid[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        instance = instance[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        dc_b = dc_b[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        return img1, img2, flow, dc_out, valid,instance,dc_b

    def __call__(self, img1, img2, flow, dc, valid,instance,dc_b):
        img1, img2 = self.color_transform(img1, img2)
        #img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, dc, valid,instance,dc_b = self.spatial_transform(img1, img2, flow, valid, dc,instance,dc_b)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)  #内存不连续存储的数组转换为连续存储的，运行速度更快
        flow = np.ascontiguousarray(flow)
        valid = np.ascontiguousarray(valid)
        dc = np.ascontiguousarray(dc)
        dc_b= np.ascontiguousarray(dc_b)
        instance = np.ascontiguousarray(instance)
        return img1, img2, flow, dc, valid,instance,dc_b
