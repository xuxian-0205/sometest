...
class Net(nn.Module):
    def __init__(self, use_stn=True):
        ...
        self.conv1d = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(p=0.5),
            nn.Conv1d(256, 50, kernel_size=1),
        )
        '''

        if self.use_stn:
            self.stn = Transform_Net(in_channels=256, out_channels=128)
    ...
    def forward(self, x):
        ...
        '''
        # x (bs,2,num_points) numpoints = 100
        batch_size = x.size(0)  # 没问题
        num_points = x.size(2)  #

        if self.use_stn:
            x0, _ = get_graph_feature(x, k=self.k)  # feature增加了C因为cat了x
            #get_f 输入x(bs,C,num_points)，输出f(bs,2C,num_points,k)
            t = self.stn(x0) #(bs,C,C)
            '''
